			...
			for miss in [5]:  #5 10;
				for misl in [1]: # 5 10;
					for maf in [0.1] : # 0.3 0.5]; 
						...
						model_result = pickle.load( open( out_dir  + "GRID_" + trans + "_" + correction  + output_string  + ".pkl" ,"rb"))
						...
						if lodo:
							...
							metrics = metrics.append({"nest":nest,"crit":crit,"maxd":maxd,"miss":miss,"misl":misl, \
								"maf":maf,"val_auc":np.mean(model_result['val_auc']),"val_f1":np.mean(model_result['val_f1']), "test_auc":np.mean(model_result['test_auc']),"test_f1":np.mean(model_result['test_f1']) },ignore_index=True)
