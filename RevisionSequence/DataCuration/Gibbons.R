...
#list_zero_cols = list()
for(k in c(5,6,7,8)){
  feature_otu = read.csv(paste0(data_dir ,"Gibbonsr_max_k",k,"/feature_table.txt"),
                   sep = "\t",stringsAsFactors = FALSE,header = TRUE)
  meta = read.csv(paste0(data_dir ,"Gibbonsr_max_k",k,"/metadata.txt"),
                         sep = "\t",stringsAsFactors = FALSE,header = TRUE)
  ...
  write.table(feature_otu ,paste0(data_dir,"Gibbonsr_max_k",k,"/metadata.txt"),sep="\t",quote=FALSE)
